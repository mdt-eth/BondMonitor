...
def generate_html_page(df_macro, df_portfolio, df_news):
    ...
    raw_text = f"1. RENDIMENTI MACRO:\n{df_macro.to_string(index=False)}\n\n2. PORTAFOGLIO:\n{df_portfolio.to_string(index=False)}\n\n3. NEWS:\n{df_news[['Fonte', 'Titolo']].to_string(index=False)}"

    # Nota: NON è una stringa formattata Python (manca la f iniziale).
    # Le variabili verranno inserite in modo sicuro dopo, usando .replace()
...
<body>
    ...
    <div class="timestamp">Notizie e Benchmark aggiornati al: __TIMESTAMP__</div>
    ...
    <div class="ai-panel">
        ...
        <p>L'intelligenza artificiale leggerà le ultime notizie di mercato e scriverà un report sugli impatti previsti per i tuoi settori target.</p>
    ...
    <pre id="raw-data" style="display:none;">__RAW_TEXT__</pre>
    ...
    <script>
        async function generateAnalysis() {
            ...
            const systemPrompt = `Sei un Portfolio Manager obbligazionario esperto.
...
1. Sintesi del sentiment macroeconomico odierno.
2. Impatto settoriale sui miei titoli.
3. Conclusione operativa.

FORMATTAZIONE: Usa ESCLUSIVAMENTE codice HTML pulito (tag <h3>, <ul>, <li>, <b>, <p>). Non inserire markdown.`;

            try {
                // Modello aggiornato senza conflitti
...
"""
    ...
    # Inserimento sicuro dei dati tramite replace()
    html_template = html_template.replace("__TIMESTAMP__", timestamp)
    html_template = html_template.replace("__HTML_MACRO__", html_macro)
    html_template = html_template.replace("__HTML_PORTFOLIO__", html_portfolio)
    html_template = html_template.replace("__HTML_NEWS__", html_news)
    html_template = html_template.replace("__RAW_TEXT__", raw_text)

    with open("index.html", "w", encoding="utf-8") as f: 
        f.write(html_template)
    print("[*] Dashboard Market Intelligence salvata con successo!")
